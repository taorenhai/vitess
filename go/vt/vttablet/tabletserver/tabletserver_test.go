...
// NewTestTabletServer is typically used in tests that
// don't need a topoServer member.
func NewTestTabletServer(config tabletenv.TabletConfig) *TabletServer {
	return NewTabletServer("test", config, nil, topodatapb.TabletAlias{})
}
...
func TestTabletServerCheckMysqlInUnintialized(t *testing.T) {
	...
	if got, want := tabletState.String(), `{"test": 0}`; got != want {
		t.Fatalf("tabletState: %s, want %s", got, want)
	}
}
